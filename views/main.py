...
from mapping.settings import start_date, end_date, MAP_URLS, min_lat, \
    max_lat, min_lon, max_lon, MAP_TYPES, MAP_KEY, BASE_LAYER
...
def health_facilities(request):
    ...
    if minlat is not None:
        facilities = \
            Facility.objects.filter(latitude__gte=Decimal(str(minlat)),
                                    latitude__lte=Decimal(str(maxlat)),
                                    longitude__gte=Decimal(str(minlon)),
                                    longitude__lte=Decimal(str(maxlon))).order_by('name'
                )

        facilities_with_reporters = [hc.facility for hc in
                HealthReporter.objects.filter(facility__latitude__gte=Decimal(str(minlat)),
    ...
    for facility in facilities:
        ...
        if zoom > 10 and type == 1:
            fac['icon'] = \
                'http://chart.apis.google.com/chart?chst=d_text_outline&chld=000000|12|l|ffffff|_|%s|'\
                 % facility.name
...
def format_date(date, start=True):
    ...
    return datetime.datetime.strptime(year + '-' + str(month) + '-01',
            '%Y-%m-%d')
    if start == False:
        ...
        if month == 0:
            month = 1
            year = year + 1
        return datetime.datetime.strptime(year + '-' + str(month)
                 + '-01', '%Y-%m-%d')
...
def deaths(request):
    ...
    if start_date == None or end_date == None:
        ...
        deathreport = DeathReport.objects.order_by('message__time'
                ).filter(valid=True)
    else:
        start_date = format_date(str(start_date))
        end_date = format_date(str(end_date), start=False)
        deathreport = \
            DeathReport.objects.filter(valid=True).order_by('message__time'
    ...
    for facility in facilities:
        ...
        try:
            dr = \
                deathreport.filter(reporter__healthreporter__facility=facility)
            if len(dr) != 0:
                death = {}

                death['title'] = 'Facility:'\
                     + str(dr[0].reporter.healthreporter.facility.name)
                death['lat'] = \
                    str(dr[0].reporter.healthreporter.facility.latitude)
                death['lon'] = \
                    str(dr[0].reporter.healthreporter.facility.longitude)
                total_deaths = dr.count()
                if total_deaths > maxvalue:
                    maxvalue = total_deaths
                death['heat'] = total_deaths
                death['desc'] = '<p><b>Deaths</b></p>total_deaths: '\
                     + str(total_deaths)
...
def births(request):
    ...
    if start_date == None or end_date == None:
        birth_reports = \
            NewBirthReport.objects.filter(valid=True).order_by('message__time'
                )
    else:
        start_date = format_date(str(start_date))
        end_date = format_date(str(end_date), start=False)
        birth_reports = \
            NewBirthReport.objects.filter(valid=True).filter(message__time__range=(start_date,
                end_date)).order_by('message__time')
    ...
    for facility in facilities:
        try:
            btrt = \
                birth_reports.filter(reporter__healthreporter__facility=facility)
            br = {}
            br['lat'] = \
                str(btrt[0].reporter.healthreporter.facility.latitude)
            br['lon'] = \
                str(btrt[0].reporter.healthreporter.facility.longitude)
            br['title'] = 'Facility: '\
                 + str(btrt[0].reporter.healthreporter.facility.name)
            birth_count = btrt.count()
            if birth_count > maxvalue:
                maxvalue = birth_count
            br['heat'] = birth_count
            br['desc'] = '<p><b>Births</b></p>Birth Reports: '\
                 + str(birth_count)
...
def malnutrition(request, start_date=None, end_date=None):
    ...
    if start_date == None or end_date == None:
        mauc = \
            MuacReport.objects.filter(valid=True).order_by('message__time'
                )
    else:
        ...
        end_date = format_date(str(end_date), start=False)
        mauc = \
            MuacReport.objects.filter(valid=True).filter(message__time__range=(start_date,
                end_date)).order_by('message__time')
    try:
        facilities = \
            Facility.objects.filter(latitude__gte=Decimal(str(minlat)),
    ...
    for facility in facilities:
        ...
        try:
            mc = \
                mauc.filter(reporter__healthreporter__facility=facility)
            mr = {}
            mr['title'] = 'Facility:'\
                 + str(mc[0].reporter.healthreporter.facility.name)
            mr['lat'] = \
                str(mc[0].reporter.healthreporter.facility.latitude)
            mr['lon'] = \
                str(mc[0].reporter.healthreporter.facility.longitude)
            mauc_count = mc.count()
            if mauc_count > maxvalue:
                maxvalue = mauc_count
            mr['heat'] = mauc_count
            mr['desc'] = '<p><b>Malnutrition</b></p>Number Of Cases:'\
                 + str(mauc_count)
...
def epi_kind(
    ...
    if start_date == None or end_date == None:
        if KIND == 'other':
            main_query = EpiReport.objects.filter(valid=True,
                    disease__in=other).order_by('message__time')
        else:
            main_query = EpiReport.objects.filter(valid=True,
                    disease=KIND).order_by('message__time')
    else:
        ...
        if KIND == 'other':
            main_query = EpiReport.objects.filter(valid=True,
                    disease__in=other).filter(message__time__range=(start_date,
                    end_date)).order_by('message__time')
        else:
            main_query = EpiReport.objects.filter(valid=True,
                    disease=KIND).filter(message__time__range=(start_date,
                    end_date)).order_by('message__time')
    ...
    for facility in facilities:
        try:
            er = \
                main_query.filter(reporter__healthreporter__facility=facility)

            if len(er) != 0:
                epi = {}
                result = er.aggregate(Sum('value'), Count('value'))
                (sum, count) = (int(result.get('value__sum')),
                                int(result.get('value__count')))
                normalized_value = sum / count
                if normalized_value > maxvalue:
                    maxvalue = normalized_value
                epi['title'] = \
                    str(er[0].reporter.healthreporter.facility.name)
                epi['lat'] = \
                    str(er[0].reporter.healthreporter.facility.latitude)
                epi['lon'] = \
                    str(er[0].reporter.healthreporter.facility.longitude)
                ...
                if kind in MAP_TYPES.keys():
                    ...
                    epi['icon'] = '/marker/25/orange/%s/marker.png'\
                         % kind
                ...
                if KIND == 'other':
                    ...
                    for k in other:
                        result = \
                            er.filter(disease=k).aggregate(Sum('value'
                                ), Count('value'))
                        try:
                            (sum, count) = (int(result.get('value__sum'
                                    )), int(result.get('value__count')))
                        ...
                        if count > 0:
                            d = \
                                '<p><b>%s</b></p><p>total number of cases:%s</p><p>number of reports: %s</p>'\
                                 % (dc[k], sum, count)
                            desc.append(d)
...
def getMessages(request):
    ...
    for message in messages:
        try:
            reporter = \
                Reporter.objects.get(connections__uri=message.uri)
        ...
        if reporter is not None and reporter.name != u'':
            try:
                ...
                msg['lat'] = \
                    str(reporter.healthreporter.facility.latitude)
                msg['lng'] = \
                    str(reporter.healthreporter.facility.longitude)
